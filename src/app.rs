...
impl App {
    pub fn new(config: Config, session_backup: Option<usize>) -> Self {
        ...
        match restored {
            Some(restored_windows) => {
                ...
                for rw in restored_windows {
                    match KovaWindow::new(event_loop, &self.config, rw.tabs, rw.active_tab) {
                        Ok(win) => {
                            let id = win.id();
            ...
            None => {
                let tab = crate::pane::Tab::new(&self.config).expect("failed to create initial tab");
                match KovaWindow::new(event_loop, &self.config, vec![tab], 0) {
                    Ok(win) => {
                        let id = win.id();
                        self.windows.insert(id, win);
                    }
